...
for(i in 1:rpt){     
  ...
  for(j in 1:cpt){   #* changes everthing to rpt instead of cpt
    ...
    r = print("r = raster('/projects/oa/tree_vel/dem/raster/ca_dem_aea.tif')")
    c = print(paste("c = crop(r, extent(",w, ",",e, ",",s, ",",n, "))", sep = ""))
    writeraster = print(paste("writeRaster(c, file = '/projects/oa/tree_vel/dem/raster/tiles/tile_", i, "_", j, ".tif')", sep = ""))
    
    combine = rbind(lib, r, c, writeraster)
    write(combine, file = paste("/projects/oa/tree_vel/dem/raster/tiles/qsub_files/tile_", i, "_", j, ".R", sep = ""))
